...
namespace ProceduralBlockWorldGeneratorToolPrototyping
{
    ...
    new Int3(3,3,3)
        ...
        {
            covers = BlockSides.none,//what this mesh would cover (which neighboring blocks touching side it would occlude)
            meshSections = new VerTriSides[] { new VerTriSides(new Vector3[0], new int[0], BlockSides.none) }//BlockSides: Stores fractions of meshes and the info on what sides of the cube need to be uncovered for them to be visible.
        };
       ...
       new VerTriSides(new Vector3[] { new Vector3(1 , 0, 0), new Vector3(1 , 1 , 0), new Vector3(1 , 1 , 1 ), new Vector3(1 , 0, 1 ) }.Select(x=>x*LandscapeTool.BlockScale).ToArray(), new int[] { 0, 1, 2, 2, 3, 0 }, BlockSides.right)
        ...
        //You are advised not to use this one. It was a technical entry only and if you make use of this, it is likely that you are applying the algorithm wrong
