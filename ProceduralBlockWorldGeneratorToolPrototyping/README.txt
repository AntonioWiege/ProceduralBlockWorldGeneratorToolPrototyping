...
Built-In Pipeline; Originally 2022.2.11f1; Test with latest 2023 version show possible shader errors and crashes with open tool inspector on play mode.
...
1) Copy the folder "ProceduralBlockWorldGeneratorToolPrototyping" into your Assets.
2) Within a scene, drag & drop the prefab "Landscape Tool Prefab" from said folder into the hierarchy and hit play.
3) Profit
...
- The brush should be configured after pressing play. This is because the values get reset to standard on play, which is also to make users understand, that the edits are temporary in this prototype version of the tool.
...
- Meta data is needed. The cone.fbx should have "convert units" disabled and two of the three normal textures are generated from grayscale.
- More detailed descriptions in the Handbook.fodt
